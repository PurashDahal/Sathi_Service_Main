...
// import 'package:singleclinic/modals/JobsByDepartmentClass.dart';
import 'package:singleclinic/screens/DoctorDetail.dart';

import '../AllText.dart';
import '../main.dart';
import '../modals/DoctorsByDepartmentClass.dart';

class JobsCategoryScreen extends StatefulWidget {
  final int id;
  JobsCategoryScreen(this.id);

  @override
  _JobsCategoryScreenState createState() => _JobsCategoryScreenState();
}

class _JobsCategoryScreenState extends State<JobsCategoryScreen>
    with TickerProviderStateMixin {
  JobsByDepartmentClass doctorsByDepartmentClass;
  List<InnerData> list = [];
  ...
  fetchJobsByDepartment() async {
    ...
    if (response.statusCode == 200 && jsonResponse['status'] == 1) {
      setState(() {
        doctorsByDepartmentClass = JobsByDepartmentClass.fromJson(jsonResponse);
      ...
      if (response.statusCode == 200 && jsonResponse['status'] == 1) {
        setState(() {
          doctorsByDepartmentClass =
              JobsByDepartmentClass.fromJson(jsonResponse);
          list.addAll(doctorsByDepartmentClass.data.data);
          nextUrl = doctorsByDepartmentClass.data.nextPageUrl;
          isLoadingMore = false;
        });
