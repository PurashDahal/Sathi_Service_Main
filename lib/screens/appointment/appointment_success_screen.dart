...
class AppointmentSuccessScreen extends StatefulWidget {
  ...
  const AppointmentSuccessScreen({Key key, this.status, this.check})
      : super(key: key);
...
class _AppointmentSuccessScreenState extends State<AppointmentSuccessScreen> {
  final JobsScopedModel doctorsScopedModel = JobsScopedModel.instance;
  final Map<String, String> individual = {"individual": "yes"};

  Future<void> fetchDataFromAPI() async {
    // Fetch data from API
    try {
      await doctorsScopedModel.getHomePageJobs(individual, isRefresh: true);
